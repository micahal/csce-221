...
private:
    ...
    void downheap(size_type index) {
        ...
        size_type max_child_idx = left_child(index);
        if (right_child(index) < c.size() && comp(c[max_child_idx], c[right_child(index)])) {
            max_child_idx = right_child(index);
        }
        if (comp(c[index], c[max_child_idx])) {
            std::swap(c[index], c[max_child_idx]);
            downheap(max_child_idx);
        }
    }
